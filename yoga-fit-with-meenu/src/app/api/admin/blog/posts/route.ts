...
import { adminListPosts, adminCreatePost, type BlogPostInput } from "@/lib/cms";
...
export async function POST(request: NextRequest) {
  let session;
  try {
    session = await requireAdmin(["admin", "editor"]);
  } catch {
    return NextResponse.json({ error: "Not authorized." }, { status: 401 });
  }
  ...
  const id = await adminCreatePost(data, session.sub);
