...
/** Packages hand-picked for the homepage "Classes & Packages" section, in
 * the admin-chosen homepage order — independent of price, id, creation
 * date, or the Pricing page's own sort_order. Only active + explicitly
 * show_on_homepage=1 packages are returned; the Pricing page's own query
 * (getActivePackages above) is untouched and keeps showing every active
 * package regardless of this selection. */
export async function getHomepagePackages() {
  const db = getDb();
  const { results } = await db
    .prepare(
      `SELECT * FROM packages WHERE is_active = 1 AND show_on_homepage = 1
