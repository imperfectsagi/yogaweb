              ...
              <article key={post.id} className="rounded-card border border-border bg-white overflow-hidden sm:flex">
                {post.featured_image && (
                  <div className="relative h-40 sm:h-auto sm:w-48 shrink-0 bg-gray-100">
                    {/* unoptimized: this app runs on Cloudflare Workers
                        (@opennextjs/cloudflare), which doesn't support
                        Next's built-in on-the-fly image optimizer — without
                        this, uploaded blog images fail to load (broken
                        image) even though the file itself is fine. Same
                        fix already applied to the homepage banner in
                        HomeBanner.tsx. */}
                    <Image src={post.featured_image} alt={post.title} fill className="object-cover" sizes="192px" unoptimized />
                  </div>
                )}
                <div className="p-6">
