...
import { adminGetPackage, adminUpdatePackage, adminDeletePackage, type PackageInput } from "@/lib/cms";
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const existing = await adminGetPackage(id);
  ...
  await adminUpdatePackage(id, data);
